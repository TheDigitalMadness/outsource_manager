...
import ways.main_menu.prices.config as cfg
...
@router.callback_query(main_menu_cfg.MainMenuWay.choose_action, F.data == "prices")
async def open_prices(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(cfg.PricesWay.choose_action)


@router.callback_query(cfg.PricesWay.choose_action, F.data == "prices_pdf")
async def send_prices_pdf(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer_document(FSInputFile(app_cfg.Files.PRICES_PDF))


@router.callback_query(cfg.PricesWay.choose_action, F.data == "audit")
...
@router.callback_query(cfg.PricesWay.choose_action, F.data == "contacts")
...
@router.callback_query(cfg.PricesWay.choose_action, F.data == "back")
